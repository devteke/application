...
export function CouponProvider({ children }: { children: ReactNode }) {
  ...
  const isPicked = (eventId: number, marketId: number, on: number) =>
  ...
  const removeSaved = (id: string) => setSaved((s) => s.filter((c) => c.id !== id))

  const value: CouponCtx = {
    active, misli, saved, totalOdd, bedel, maxWin,
    isPicked, pick, remove, clear, setMisli, save, removeSaved,
  }
