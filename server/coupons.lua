...
local function onCooldown(src, key, ms)
    ...
    cd[src] = cd[src] or {}
    if cd[src][key] and now < cd[src][key] then
        return true
    end
    cd[src][key] = now + ms;
    return false
...
local function newId()
  return ('%d-%06d-%04x'):format(os.time(), math.random(0, 999999), math.random(0, 0xffff))
end
...
function Coupons.place(src, payload)
    if onCooldown(src, 'place', App.Config.PlaceCooldownMs) then
    ...
    if type(sel) ~= 'table' then
        return false, 'invalid_bets'
    end
    local n = #sel
    if n < App.Config.MinSelections or n > App.Config.MaxSelections then
    ...
    for _, b in ipairs(sel) do
        if type(b) ~= 'table' or type(b.eventId) ~= 'number' or type(b.marketId) ~= 'number' or type(b.on) ~= 'number' then
            return false, 'invalid_selection'
        end
        if seen[b.eventId] then
    ...
    if btype == 'system' then
        if n < 3 then
            return false, 'system_min_selections'
        end
        ...
        local B, M = #bankos, #nb
        if M < 2 then
            return false, 'system_needs_nonbanko'
        end -- en az 2 banko-dışı maç

        local bankoProd = 1.0
        for _, r in ipairs(bankos) do
            bankoProd = bankoProd * r.odd
        end
        ...
        for _, k in ipairs(sizes) do
            if type(k) ~= 'number' or k ~= math.floor(k) or k < 1 or k > M then
            ...
            if not seenK[k] then
                ...
                for _, r in ipairs(bankos) do
                    if (r.mbs or 1) > T then
                        return false, 'mbs_not_met'
                    end
                end
        ...
        if App.Config.MaxOdd > 0 and totalOdd > App.Config.MaxOdd then
            return false, 'odd_too_high'
        end
        for _, r in ipairs(resolved) do
            if (r.mbs or 1) > n then
                return false, 'mbs_not_met'
            end
        end
        combos = 1
    ...
    if App.Config.MaxStake > 0 and stake > App.Config.MaxStake then
        return false, 'stake_too_high'
    end
    if App.Config.MaxWin > 0 and maxWin > App.Config.MaxWin then
        return false, 'win_too_high'
    end
    ...
    if not App.Economy.removeMoney(src, stake, 'bahis-kupon') then
    ...
    -- Kupondaki en geç başlangıç (settle bundan önce boşuna denemesin)
    local dueAt = 0
    for _, r in ipairs(resolved) do
        if type(r.startsAt) == 'number' and r.startsAt > dueAt then
            dueAt = r.startsAt
        end
    end
    ...
    local ok = pcall(function()
        MySQL.insert.await(
            'INSERT INTO betting_coupons (id,identifier,misli,stake,total_odd,max_win,status,bets,meta,created_at,settle_due_at) ' ..
                'VALUES (?,?,?,?,?,?,?,?,?,?,?)',
            {id, ident, misli, stake, totalOdd, maxWin, 'pending', json.encode(bets), meta, now, dueAt})
    end)
...
function Coupons.delete(src, payload)
    ...
    local affected = MySQL.update.await("UPDATE betting_coupons SET hidden = 1 " ..
...
local function settleOnce()
  ...
  -- Sadece başlangıcı gelmiş (veya eski/NULL) pending kuponlar
  local rows = MySQL.query.await(
    "SELECT id,identifier,misli,stake,bets,meta FROM betting_coupons "
    .. "WHERE status='pending' AND (settle_due_at IS NULL OR settle_due_at <= ?) "
    .. "ORDER BY settle_due_at ASC LIMIT 200", { nowMs }) or {}
  ...
  for statId, sport in pairs(need) do
    local data
    if App.Config.UseTestStats then
      ...
      if override then
        ...
        if okd and dec then data = dec.data or dec end
      end
    end
    if data == nil then
      local res = App.Http.get((App.Config.StatsPath):format(sport, statId))
      if res.ok and res.body and res.body.data then data = res.body.data end
      Wait(150)
    end
    stats[statId] = data
  end
    ...
    for _, r in ipairs(rows) do
        ...
        if not pending then
            local meta = r.meta and json.decode(r.meta) or {
                type = 'combi'
            }
            ...
            if meta.type == 'system' then
                ...
                payout = 0
                if result == 'won' then
                    payout = math.floor(stake * App.Settle.settledOdd(bets, perBet) + 0.5)
            ...
            if payout > 0 then
                App.Economy.payByIdentifier(r.identifier, payout, 'bahis-' .. result)
            end
            MySQL.update.await(
...
CreateThread(function()
    while true do
        ...
        if not ok then
            App.log('error', 'settle: ' .. tostring(err))
        end
    end
end)
